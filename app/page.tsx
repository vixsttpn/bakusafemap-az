...
type RouteStep={instruction:string,distance:number,duration:number,location:[number,number]};
...
export default function Page(){
  ...
  const [pos,setPos]=useState<{lng:number,lat:number,heading:number,speed:number,acc:number}|null>(null);
  ...
  const speak=(text:string)=>{
    ...
    u.lang='ru-RU'; u.rate=1; u.pitch=0.9;
    const voices=window.speechSynthesis.getVoices();
    const male=voices.find(v=>v.lang.includes('ru')&&/(Pavel|Dmitry|Yuri)/i.test(v.name)) || voices.find(v=>v.lang.includes('ru')) || voices[0];
      ...
      map.on('load',()=>{
        ...
        if(navigator.geolocation){
          watchRef.current=navigator.geolocation.watchPosition((p)=>{
            ...
            setPos({lng:now.lng,lat:now.lat,heading,speed,acc:p.coords.accuracy});
  ...
  const fetchRoute=async(from:{lng:number,lat:number},to:{lng:number,lat:number})=>{
    ...
    const s:RouteStep[]=route.legs[0].steps.map((st:any)=>({instruction:st.maneuver.type==='depart'?'Начните движение':st.maneuver.type==='arrive'?'Вы прибыли':`${st.name||'дорогу'}`,distance:st.distance,duration:st.duration,location:st.maneuver.location}));
    setSteps(s); setCurStep(0); setNavigating(true);
    speak(`Маршрут ${Math.round(route.distance/1000)} км, ${Math.round(route.duration/60)} мин. ${s[0]?.instruction}`);
  };

  const addReport=(type:Report['type'])=>{
    const center=mapRef.current?.getCenter();
    const lng=center? center.lng : (pos? pos.lng : 49.854);
    const lat=center? center.lat : (pos? pos.lat : 40.376);
    ...
    div.innerHTML=`<div style="width:18px;height:18px;background:${col};border:2px solid white;border-radius:50%"></div>`;
    if(mapRef.current){
      (async()=>{
        const ml=await import('maplibre-gl');
        new ml.Marker({element:div.firstChild as HTMLElement}).setLngLat([lng,lat]).addTo(mapRef.current);
      })();
    }
  };
  ...
  return(
    <div className="relative w-full h- bg-[#f2f2f2] overflow-hidden">
      ...
      <div className="absolute top-3 left-3 right-3 h- bg-white rounded-full shadow flex items-center px-2 z-10">
      ...
      <div className="absolute right-3 top- flex flex-col gap-3 z-10">
        ...
        <button onClick={()=>setShowReport(true)} className="w-11 h-11 bg-[#ff3b30] text-white rounded- shadow flex items-center justify-center">!</button>
        <button onClick={()=>setVoiceOn(v=>!v)} className={`w-11 h-11 rounded- shadow flex items-center justify-center ${voiceOn?'bg-white':'bg-gray-300'}`}><IconVoice/></button>
      </div>
      {showReport && (
        <div className="absolute inset-0 bg-black/40 z-30 flex items-end">
          <div className="w-full bg-white rounded-t- p-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button onClick={()=>addReport('danger')} className="h-16 rounded-xl bg-red-50 border">🚨 Опасность</button>
              <button onClick={()=>addReport('police')} className="h-16 rounded-xl bg-blue-50 border">👮 Полиция</button>
              <button onClick={()=>addReport('accident')} className="h-16 rounded-xl bg-orange-50 border">💥 ДТП</button>
      ...
      {navigating && steps[curStep] && (
        <div className="absolute bottom-3 left-3 right-3 bg-white rounded- shadow p-3 z-20">
          <div className="flex items-center justify-between">
            <div><div className="text- text-gray-500">{Math.round(steps.slice(curStep).reduce((a,b)=>a+b.distance,0)/1000*10)/10} км</div><div className="text- font-bold">{steps[curStep].instruction}</div><div className="text- text-gray-600">{steps[curStep].distance.toFixed(0)} м</div></div>
